    ...
    <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full font-sans">
      ...
      <div className="text-center mb-12 space-y-3">
        <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-[#0a2319] border border-[#e8c872]/35 text-[#fbf5e6] text-xs uppercase tracking-[0.25em] backdrop-blur-md">
          ...
          <span>Original Artworks &amp; Fine Art Prints</span>
        </div>
        <h1 className="font-display text-4xl sm:text-5xl text-zinc-100 font-light tracking-tight">
          Original Fine Art &amp; <span className="font-signature text-5xl sm:text-7xl text-[#fbf5e6] drop-shadow-[0_0_25px_rgba(232,200,114,0.45)]">Catalogue</span>
        </h1>
        <p className="text-xs sm:text-sm text-[#a3b8af] max-w-lg mx-auto leading-relaxed">
          Each artwork is rendered on archival canvas or heavyweight ivory paper, accompanied by our signature handwritten wax-sealed scroll.
        </p>
      </div>
      ...
      <div className="space-y-6 mb-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Search by title, medium, character..."
              value={search}
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-end">
            ...
            <select
              ...
              className="bg-[#0a2319] border border-emerald-900/80 rounded-xl px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:border-[#e8c872]"
            >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-[#0a2319]/50 border border-emerald-900/60 rounded-3xl space-y-3">
          ...
          <p className="text-xs text-zinc-400">Try adjusting your search criteria or resetting filters.</p>
          <button
            onClick={() => { setActiveCategory('all'); setSearch(''); }}
            className="bg-[#e8c872] text-black px-6 py-2 rounded-full text-xs uppercase tracking-wider font-semibold inline-block mt-2 btn-magnetic"
          >
            Show All Artworks
          </button>
        </div>
